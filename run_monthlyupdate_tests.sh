...
python defend_data_capture/manage.py testserver \
    ...
    & echo $! > backend.pid \
    & (sleep 5 && npx cypress run --browser chrome --config-file false -c video=false --config baseUrl=http://localhost:8001 --spec "cypress/integration/monthly_update_forms_*.js")
